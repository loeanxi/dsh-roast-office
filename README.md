...
The standalone plugin does not append unknown event types to dsh Session logs: doing so would make older dsh versions reject a session during replay. The native dsh Web adapter therefore uses the existing command lifecycle as its compatibility channel: `/review` returns a structured, log-safe result that renders as a review card without changing the Session event vocabulary. A dsh-specific adapter may additionally persist the richer protocol after declaring its event types and compatibility policy; the core plugin remains usable by CLI, headless, and other hosts without modifying their session format.
