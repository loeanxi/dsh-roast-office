...
The host can connect a separate reviewer session with `installIndependentReviewer(ctx, reviewer)`. The injected reviewer receives only the `ReviewRequest` evidence and returns a summary plus findings; the bridge emits `roast-office/review-result` for the Web card. The host is responsible for creating that reviewer Agent with read-only tools and a separate session. The execution Agent is never passed to the reviewer as its conversation.
