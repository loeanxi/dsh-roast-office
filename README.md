...
The host can connect a separate reviewer session with `installIndependentReviewer(ctx, reviewer)`. The injected reviewer receives only the `ReviewRequest` evidence and returns a summary, findings, confidence, non-sensitive evidence, and a `needsSecondReview` flag; the bridge emits `roast-office/review-result` for the Web card. The host is responsible for creating that reviewer Agent with read-only tools and a separate session. The execution Agent is never passed to the reviewer as its conversation.
...
The standalone plugin does not append unknown event types to dsh Session logs: doing so would make older dsh versions reject a session during replay. The native dsh Web adapter therefore uses the existing command lifecycle as its compatibility channel: `/review` returns a structured, log-safe result that renders as a review card without changing the Session event vocabulary, and automatic review results use the same known command pair. When a dsh host exposes `subagents`, the default `spawn` provider is mounted as soon as that host service becomes available and runs the review in a separate Agent session; hosts without that capability use the deterministic reviewer for the active command. A dsh-specific adapter may additionally persist the richer protocol after declaring its event types and compatibility policy; the core plugin remains usable by CLI, headless, and other hosts without modifying their session format.

The observer detects `repeat-call` when one Agent invokes the same tool with canonicalized identical arguments consecutively. It reconstructs the same metrics from known `tool/call` and `tool/result` events when a historical dsh session is reopened, so `/review` remains useful after a page reload. It detects `failed-retry` when the same tool and failure code/message repeat consecutively. A mutation only enters the verification window when one of its path-like arguments matches `verificationPaths`; documentation-only paths such as `README.md` are ignored by default. `clean-finish` is emitted to the console when an agent becomes idle after at least one call and includes calls, failures, repeat incidents, failure retries, unique tools, mutations, verification runs, unverified changes, score, risk, and verdict.
